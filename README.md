...
这是一个可运行的 Python 项目，用双层引擎为 `SH.515450` 输出五类模型读数：综合评分、估值吸引力、过热风险、日内波动状态和模型仓位示例。系统只用于非商业研究与状态展示，不会下单，也不构成投资建议。
...
- 估值吸引力：战略价值为主，结合盘中偏离、VWAP 和相对异常。
- 过热风险：战略价值偏低且出现 RSI/VWAP/日内涨幅过热时升高。
- 日内波动状态：聚焦 VWAP 偏离、相对基准强弱、日内涨跌幅和盘口失衡。
- 模型仓位示例：由综合评分进行数值映射，仅用于解释模型，不构成仓位建议。

估值吸引力与过热风险不是简单互补：长期价值高但盘中快速拉升时，两者可能同时处于中位区间。
